...
def main():
    ...
    # 開始ログ出力
    logPut('\n')
    logPut('北海道案件検索処理開始 ' + datetime.datetime.now()
    ...
    with open(urlPath + 'url.csv', 'r', encoding='shift_jis') as csvfile:
        ...
        for uRow in url_csv:
            ...
            if os.path.exists(beforePath + scrapingType + '.csv'):
                for nRow in result:
                    # 前回確認結果の取得
                    with open(beforePath + scrapingType + '.csv', 'r',
                              encoding='utf-16') as csvfile2:
                        before_csv = csv.reader(
                            csvfile2, delimiter=',', quotechar='"')
            ...
            # 差分結果パスに書き込み
            with open(differencePath + scrapingType + '.csv', 'w',
                      encoding='utf-16', errors="ignore") as w:
                writer = csv.writer(w, lineterminator='\n')
                writer.writerows(differenceList)

            # サマリ用
            for difference in differenceList:
                difference_all.append(
                    [name, url+' ', difference[0], difference[1]])

            # 前回確認結果パスに書き込み
            with open(beforePath + scrapingType + '.csv', 'w',
                      encoding='utf-16', errors="ignore") as w:
                writer = csv.writer(w, lineterminator='\n')
                writer.writerows(result)

    # 差分結果パスに書き込み(サマリ用)
    with open(differencePath + csv_file_name, 'w', encoding='utf-16',
              errors="ignore") as w:
        writer = csv.writer(w, lineterminator='\n')
    ...
    # 終了ログ書き込み
    logPut('北海道案件検索処理終了 ' + datetime.datetime.now().
           strftime('%Y/%m/%d %H:%M:%S'))
...
def scraping(name, scrapingType, url):
    ...
    Args:
        name (str): 事業者名
        scrapingType (str): 事業者の種別(spmdu、spsdkなど)
        url (str): URL
...
def getCsvPath(getName):
    ...
    Args:
        getName (str): 事業者の種別(spmdu、spsdkなど)
...
def target_click(url, tag_type, target):
    ...
    Args:
        url (str): 操作するURLアドレス
        tag_type (str): ボタンの要素種別(class、id、href...)
        target (str): ボタンの要素名(class名、id名等)
    """
    # ChromeDriver読み込み
